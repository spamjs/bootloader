select_namespace("utils.custom.navigation", function(navigation){
	...
	navigation.widget_on_focus = function(elem, uE, e) {
        return 'select';
    };
    navigation.widget_on_blur = function(elem, uE, e) {
        return 'none';
    };
    navigation.widget_on_keydown = function(elem, uE, e) {
        if (utils.is.enter(uE.key)) {
        	uE.key = utils.is.down(); return 'select';
        } else if(uE.readOnly) return 'select';
        else return uE.mode;
    };
    navigation.widget_on_modechange = function(elem, uE, e) {
	...
	navigation.tagEventHandler = function(elem,uE,e,defaultMode){
        ...
        if(uE.readOnly && e.isKeyDownEvent){
        	newmode = navigation.widget_on_keydown(elem,e.uE ,e)
