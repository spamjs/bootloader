select_namespace("utils.custom.grid", function(grid){
	//supports diff 
	grid.init = function(){
	...
	grid.GRIDS = {};
	grid.define = function(name, initCB) {
		return this.GRIDS[name] = initCB;
	};
		...
		if(struct.tagType=="button" || struct.tagType=="arrowtoggle"){
			struct.onClick = function(e){
				if(!e.$widget.hasClass("dead")){
					...
					if(GRID["_onclick"]) GRID._onclick(e);
					if(GRID[fieldType_onclick]) GRID[e.fieldType+"_onclick"](e);
				}
			};
		} /*else {
			struct.onChange = function(e,uE){
				...
				if(!e.$widget.hasClass("dead")){
					...
					} else GRID._setValue(e.iRow,e.iCol,e.widgetValue);
					var fieldType_onchange = e.fieldType + "_onchange";
					if(GRID[fieldType_onchange]) GRID[fieldType_onchange](e);
					if(e.curChange){
    ...
    grid.setOnChange = function(GRID){
    	GRID.$dataDiv.onChange(function(e,uE){
			if(!uE.$widget.hasClass("dead")){
				GRID._setValue(uE.iRow,uE.iCol,uE.$widget.getValue());
				var  fieldType = uE.fieldType;
				var fieldType_onchange = uE.fieldType + "_onchange";
				if(GRID[fieldType_onchange]) GRID[fieldType_onchange](uE);
				if(uE.curChange){
					//e.$tagwrapper = e.$widget.parents(".tag.amount");
					//e.iRow = e.$tagwrapper.attr("iRow");
					//e.iCol = e.$tagwrapper.attr("iCol");
					//e.ID = this.onChange.ID
					uE.index = GRID.vertical ? uE.iRow : uE.iCol;
					uE.fieldType = e.fieldType + "_cur"; //e.$widget.attr("fieldType");
					GRID._setValueByFieldType(uE.index,uE.fieldType,uE.curValue);
					var fieldType_onchange = uE.fieldType + "_onchange";
					if(GRID[fieldType_onchange]) GRID[fieldType_onchange](uE,e);
					//if(GRID["_onchange"]) GRID._onchange(e);
				}
				uE.fieldType = fieldType;
				if(GRID["_onchange"]) GRID._onchange(uE,e);
			} 
		})
    };
...
//		var headerWidth = 0;
	...
	grid.getHeaderCell = function(GRID,j){
		var $header = ""; var fieldType = ""; var tagType = "";
		var $cell = "";
		var style = "";
		var title = "";
		if(GRID.structure[j]){
			...
			if(struct.$header){
				...
				struct.$header.row =struct.$header.iRow = -1;
				struct.$header.col = struct.$header.iCol = j;
				if(__val) struct.$header.value = __val; 
				if(struct.$header.formatter)  struct.$header.value = struct.$header.formatter(struct.$header.value,{ iRow : -1, iCol : j});
				$header = utils.custom.tag(grid.getStruct(struct.$header,GRID));
			}
		}
		if(struct.width) style = style + ' width:'+struct.width+'px';
		if(struct.title) title = title + ' title = "'+struct.title+'"';
		var tag = (GRID.useTH)? 'th' : 'td';
		$cell = (new utils.custom.dom('div')).addClass('ft_'+fieldType+' tt_'+tagType).attr('style',style).append($header);
		//$('<div class=" ft_'+fieldType+' tt_'+tagType+'" style="'+style+'"></div>').append($header);
		$cell = (new utils.custom.dom(tag)).addClass('customGridCell ft_'+fieldType+' tt_'+tagType)
			.attr('style',style).attr(title).append($cell.toString());
			//$('<'+tag+' class="customGridCell ft_'+fieldType+' tt_'+tagType+'" style="'+style+'"'+title+'></'+tag+'>').append($cell);
		return $cell;
	};
	
	grid.refresh = function(GRID){
		GRID.parent.addClass("navigate").attr("tabindex",0).attr("maxrow",GRID.getRowCount()+2); GRID.parent.attr("maxcol", GRID.getColCount()+2);
		if(GRID.hideDeadRow){
			var totalRows = GRID.getRowCount();
			if(GRID.getColCount()){
				for(var j=0; j<totalRows; j++){
					...
					} else $('.customGridRow.rowNum_'+j,GRID.$tables).removeClass("dn");
				} 
			} else {
				for(var j=0; j<totalRows; j++){
					var __struct = GRID.getStructure(j,0);
		...
		if(GRID.onRefresh) GRID.onRefresh();
...
//		}
	...
	grid.getCell = function(GRID,i,j,_struct){
		var h,index;
		if(GRID.vertical) {h=j;index=i} else {h=i;index=j};
//		/var index = GRID.horizontal ? j : i;
		if(_struct==undefined){
			var __struct = GRID.getStructure(i,j);
			if(__struct) GRID._struct = __struct;
			...
			if(_altStruct.tagType=='amount' && GRID.bulkType){
				...
				if(_curValue)  _altStruct.curValue =  _curValue;
			}
			_altStruct.row = _altStruct.iRow = i;
			_altStruct.col = _altStruct.iCol = j;
		}
		_altStruct.tabIndex = GRID.tabIndex;
		var $cell = (new utils.custom.dom('td')).addClass('customGridCell ft_'+_altStruct.fieldType+' tt_'+_altStruct.tagType+' cid_'+_altStruct.id)
			//$('<td class="customGridCell ft_'+_altStruct.fieldType+' tt_'+_altStruct.tagType+' cid_'+_altStruct.id+' "></td>');
		if(GRID){
		...
		if(_altStruct.formatter)  _altStruct.value = _altStruct.formatter(_altStruct.value,{ iRow : i, iCol : j});
		$cell.html(utils.custom.tag(_altStruct)+"");
		grid.defaultCellStructure.value = "";
		return $cell;
	};
	
	grid.checkFree = function(GRID){
		if(GRID.freeQ.size()>0 && GRID._col_add==0 && GRID._row_add==0){
			GRID.freeQ.executeStart();
		}
	}
	grid.addColQ = utils.queue(function(rep){
		var totalRows = rep.GRID.getRowCount();
		for(var i=0; i<totalRows;i++){
			$('.customGridRow.rowNum_'+i, rep.GRID.$table).append(grid.getCell(rep.GRID,i,rep.j).toString());
		}
		grid.refresh(rep.GRID);
		rep.GRID._col_add--;
		grid.checkFree(rep.GRID);
	},10);
	grid.addRowQ = utils.queue(function(rep){
		var GRID = rep.GRID,i = rep.i,reverse = rep.reverse;
		var $row = (new utils.custom.dom('tr')).addClass('customGridRow rowNum_'+i);//$('<tr class="customGridRow rowNum_'+i+'"></tr>');
		if(GRID.hasHeaders && GRID.horizontal){
			var _$row = (new utils.custom.dom('tr')).addClass('customGridRow rowNum_'+i); // $('<tr class="customGridRow rowNum_'+i+'"></tr>');
			_$row.append(grid.getHeaderCell(GRID,i).toString());
			GRID.$header.append(_$row.toString());
		}
		
		var totalCols = GRID.getColCount();
		for(var j=0; j<totalCols; j++){
			$row.append(grid.getCell(GRID,i,j).toString());
		}
		if(reverse) $(".customGridRowHeader",GRID.$table).after($row);
		else GRID.$table["append"]($row.toString());
		GRID._row_add--;
		grid.checkFree(GRID);
	},10);
	grid.addCol = function(GRID,j){
		GRID._col_add++
		grid.addColQ.put({GRID : GRID,j : j},!GRID.lazyY);
	};
	...
	grid.addRow = function(GRID,i,reverse){
		GRID._row_add++;
		return grid.addRowQ.put({
			GRID : GRID,i :i,reverse : reverse
		},!GRID.lazyX)
	};
	grid.deleteRow = function(GRID,i){
		$('.customGridRow.rowNum_'+i,GRID.$tables).remove();
		GRID._deleteRow(i);
	};
	
	grid.get = function(GRID,arg0,arg1,arg2,arg3,arg4){
		if(GRID.name){
			utils.require('/js/grids/' + GRID.name + ".js");
			if (grid.GRIDS[GRID.name])
				grid.GRIDS[GRID.name](GRID,arg0,arg1,arg2,arg3,arg4);
		}
		GRID.freeQ = utils.queue(function(cb){
			if(cb) cb(GRID);
		}); GRID.freeQ.blocked = true;
		GRID.ifReady = function(cb){
			GRID.freeQ.put(cb);
		};
		...
		GRID.clear = function(){
			this.set({ data : []});
			if(this.structureRefresh) this.structureRefresh();
			return grid.refresh(this);
		};
		...
		GRID.__setValue = function(r,c,value){
			if(!GRID.data) GRID.data = [];
			if(GRID.data[r]==undefined) GRID.data[r] = [];
			GRID.data[r][c] = value;
		};
		
		GRID.addRow = function(rowList, reverse){
			if(rowList==undefined) var rowList= [];
			var totalCols = GRID.getColCount();
		...
		GRID.addCol = function(rowList,j){
			if(rowList==undefined) var rowList= [];
			var totalRows = GRID.getRowCount();
			var curCol = Math.min(GRID.getColCount(),j);
			for(var i=0; i<totalRows;i++){
				GRID._setValue(i,curCol,rowList[i]);
			}
			if(curCol<GRID.getColCount()){
				grid.refresh(GRID);
			} else grid.addCol(GRID,curCol);
			return curCol;
		};
		...
		if(GRID.horizontal) GRID.vertical = false;
		else GRID.vertical = true;
		if(GRID.navigate==undefined) GRID.navigate=true;
		if(GRID.displayData) GRID.hasDisplayMap = true;
		else GRID.displayData = [];
		if(GRID.cellpadding==undefined)  GRID.cellpadding = 0;
		if(GRID.cellspacing==undefined)  GRID.cellspacing = 1;
		if(GRID.focus==undefined) GRID.focus = {};
		GRID._row_add = 0;GRID._col_add = 0;
		GRID.getParam = function(i,key){
			if(key==undefined) var key = "fieldType";
			if(this.structure[i]) return this.structure[i][key];
			return null;
		};
		...
		GRID.setFocus = function(r,c){
			if(r!=undefined && c!=undefined){
				...
				this.focus.r = null;this.focus.c  = null;
			}
		};
		//$ DOM searchfunctions
		...
		GRID.$byFieldType = function(index,fieldType){
			if(this.horizontal){
				if(index<0) return $("td .ft_"+fieldType+" .tag",this.$header)
				return $(".tag."+fieldType+".c"+index,this.$table);
			} else return $(".tag."+fieldType+".r"+index,this.$table);;
		};
		//
		GRID.setValue = function(rowNum,colNum,value){
			if(rowNum>=0) this._setValue(rowNum,colNum,value);
		...
		GRID.setValueByFieldType = function(index,fieldType,value){
			...
			} else $tag = $(".tag."+fieldType+".r"+index,this.$table);
			var _value = value;
			if($tag && $tag.length){
//				var colNum = $tag.attr('iCol')-0;
//				var rowNum = $tag.attr('iRow')-0;
//				if(rowNum>=0) GRID._setValue(rowNum,colNum,value);
		...
		GRID.forAllRows = function(_cb){
			...
			this._cb = _cb;
			for(var i=0; i<rows; i++) this._cb(i);
		};
		GRID.forAllColumns = function(_cb){
			var cols = this.getColCount();
			this._cb = _cb;
			for(var i=0; i<cols; i++) this._cb(i);
		};
		...
		if(GRID.bulkType) grid.getBulkGrid(GRID);
		else grid.getNormalGrid(GRID);

		if(GRID) GRID.set(GRID);
		return GRID;
	};
	...
	grid.getBulkGrid = function(GRID){
		...
		GRID.cidcounter = 0;
		GRID.addCol = function(bulk,j){
			if(bulk==undefined) var bulk= {};
			var totalRows = GRID.getRowCount();
			var totalCols = GRID.getColCount()
			var curCol = Math.min(totalCols,j);
			if(this.maxCols!==undefined && curCol>=this.maxCols){
				return;
			}
			if(curCol<totalCols){
				if(curCol>=0) GRID.data[curCol] = {};
		...
		GRID.getValue = function(rowNum,colNum){
			...
			if(colNum<0) return GRID.headerData[fieldType];
		...
		GRID._setValue = function(rowNum,colNum,value){
			if(!GRID.structure[rowNum]) return;
			var fieldType = GRID.structure[rowNum].fieldType;
	...
	grid.getNormalGrid = function(GRID){
		if(GRID.pivotCol==undefined) GRID.pivotCol=0;
		if(!GRID.data[0]) GRID.data[0] = [];

		if(GRID.vertical){
			GRID.getRowCount = function(_col){
				if(!_col) var _col = GRID.pivotCol;
	...
	grid.format = {
		formatTypes : {},
		set : function(key,cb){
			if(this.formatTypes[key]) LOG.warn('format type overridden',key);
