			...
			if (this.content[dEvent.path]) {
				for(var i in this.content[dEvent.path]){
					...
					if(formatType && TEMP[formatType]){
						dEvent.value = TEMP[formatType](dEvent.value,$(elem))
					} 
	...
	template.Template  = function Template(OBJ){
		...
		this.tag_onchange = function(uE,e) {
			if(uE.rx.path){
				...
				this.ODO.change(uE.rx.path, uE.$widget.getValue());
				if(this.ODO.SLAVE) 
					this.ODO.SLAVE._callFun(uE.rx.path,uE.$widget.getValue());
			}
			if(uE.rx.change){
			...
			if (OBJ.$div) {
				...
				if (OBJ.cached && template.HTML_CACHE[OBJ.template]) {
					...
					OBJ.success = function(html, $html) {
						...
						this.$div = $(html);
						template.$div_onready(this,this.$div);
					}
					OBJ.method = "POST";
