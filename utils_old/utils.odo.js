var ODO = function(_fromData){
	...
	this.dataEvent = function dataEvent(index,keys,key,value){
		...
		this.merge = function(dEvent){
			if(this===dEvent) return this;
			this.push(dEvent.path);
		}
	}
	this.blockPath = function(path){
		this.replace_path[path] = true; return this;
	}
	...
	this.set = function(_key, finalValue){
		...
		_data[_onKey] = (finalValue===undefined) ? _data[_onKey] : finalValue;
	};
	...
	this.get = function(_key){
		if(_key==undefined) return this.data;
		var key = this.refineKey(_key);
		this.getRef(key);
		return _data[_onKey];
	};
	...
	this._update = function(_target, src, path){
		...
		if ((options = src) != null) {
			...
			for ( var name in options) {
				if(name.indexOf(':')==0){
					this.event_map[path+name] = options[name];
				} else {
					...
					if (copy && typeof copy === "object" && !copy.nodeType) {
						...
						if(!this.replace_path[nextPath]){
							target[name] = this._update(src || (copy.length != null ? [] : {}), copy,nextPath+(copy.length != null ? '#' : '.'));
						} else {
							target[name] = copy;
	...
	//TODO:- executeOnce is supposed to make function executable only once if passed true, for all the changes for that key
	this.sub = function(_KEY, fun,executeOnce,block){
		var _KEYS = _KEY.split(',');
		var FUN = (!executeOnce ? fun.bind(this) : utils.execute.define(fun,this)
				.prop('onOver',function(){
		...
		for(var i in _KEYS){
			if(_KEYS[i].indexOf(':')==0){
				this.onchange_fun.events[_KEYS[i]] = FUN;
			} else{
	...
	this._sub = function(_KEY, FUN,executeOnce,block){
		...
		ref['@' + _nextKey] = {
			//paths : [],
			key : _nextKey, next : FUN, nextKey : null,executeOnce :executeOnce, eventKey : _KEY
		};
	};
	...
	this._callFun = function(key,val){
		...
		if (this.onchange_fun.next) {
			this.onchange_fun.next(new this.dataEvent(0, keys,key,val),this.data,this);
		}
	...
	this.next = function(dEvent, __data,THIS){
		...
		if (this['@' + __key]) {
			nextThis = this['@' + __key];
			//return  this['@' + __key].next(list, index + 1, keys,e,THIS,__data)
		} else if (this['@*']) {
	...
	this.trigger = function(){
		...
		for(var key in this.event_map){
			if(this.onchange_fun.events[key]){
				this.onchange_fun.events[key](this.event_map[key]);
				delete this.event_map[key]
			}
		}
		//delete refs...???
	};
